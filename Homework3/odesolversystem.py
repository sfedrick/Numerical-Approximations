...
def eulerstep(yn,tn,f,h):
    ...
    for i in range(0,len(yn)):
        new=yn[i]+h*f(yn,tn,i)
        yn1.append(new)
    return yn1
def odesysE(x0,a,b,N,f):
    h=abs(a-b)/N
    ...
    while(end<=b):
        ...
        #sends input to Euler system algorithm
        new=eulerstep(input,t[-1],f,h)
        i=0
        for list in X:
    ...
    return X,t
def leapfrogstep(yn0,yn,tn,f,h):
    yn1=[]
    for i in range(0,len(yn)):
        new=yn0[i]+2*h*f(yn,tn,i)
        yn1.append(new)
    return yn1

# take in a system of odes
def odesysLF(x0,a,b,N,f):
    h=abs(a-b)/N
    #x0 is a vector of init values
    #a and b are the beginning and end of the range
    #h is the step size
    ...
    while(end<=b):
        if(check2-check1>PercentCheck):
            check1=float(end)/b
            print("percent until Leapfrog completes "+str(check1))
        input=[]
        for list in X:
        ...
        if(not start):
            ...
            for list in X:
                #this creates the array that is fed into the rk4 algorithm
                input0.append(X[list][-2])
            new=leapfrogstep(input0,input,t[-1],f,h)
        ...
        check2=float(end)/b
    return X,t
def leapfrogsteplag(yn0,yn,tn,f,h):
    yn1=[]
    for i in range(0,len(yn)):
        new=yn0[i]+2*h*f(yn0,yn,tn,i)
        yn1.append(new)
    return yn1
def rk4steplag(yn,tn,f,h):
    #note that k1 k2 k3 and k4 are lists the length of these lists
    #corresponds to the number of odes in your system
    a=1/2.0
    k1=h*rk4helplag(yn,tn,f)
    k2=h*rk4helplag(yn+a*k1,tn+h/2.0,f)
    k3=h*rk4helplag(yn+a*k2,tn+h/2.0,f)
    k4=h*rk4helplag(yn+k3,tn+h,f)
    yn1=yn+(1/6.0)*k1+(1/3.0)*(k2+k3)+(1/6.0)*k4
    return yn1
def rk4helplag(yn,tn,f):
    #helper function that finds the k value from each function and appends it to K
    k=[]
    for i in range(0,len(yn)):
        k.append(f(yn,yn,tn,i))
    k=np.array(k)
    return k
# take in a system of odes
def odesysLFlag(x0,a,b,N,f):
    h=abs(a-b)/N
    #x0 is a vector of init values
    #a and b are the beginning and end of the range
    #h is the step size
    ...
    while(end<=b):
        ...
        if(not start):
            ...
            for list in X:
                #this creates the array that is fed into the rk4 algorithm
                input0.append(X[list][-2])
            new=leapfrogsteplag(input0,input,t[-1],f,h)
